...
using namespace std;
ofstream outfile;
#define machinenumber 6  //机器的总数（等于每道工序的并行机个数×工序数），如第一道工序并行两台机器，第二道工序并行两台机器，第三道工序并行两台机器
...
int initialization()   //初始化种群；
{
    ...
    for(int i=0; i<populationnumber; i++)     //将全排列的个体中随机选取两个基因位交换，重复工件个数次，以形成随机初始种群；
        for(int j=0; j<workpiecesnumber; j++)
        {
            ...
            int flg3=a[i][flg1];
            a[i][flg1]=a[i][flg2];
            a[i][flg2]=flg3;//将第i个染色体的某两个基因交换位置
        }

    cout<<"InitialPopulation："<<endl;
...
int fitness(int c)   //计算适应度函数，c代表某个体；a[c]即为第i个染色体
{
    ...
    makespan=totaltime;
    fits[c]=1.000/makespan;          //将makespan取倒数作为适应度函数；makespan越大则适应度越小
}
...
int select()
{
   ...
   for(int i=0;i<populationnumber;i++)
   {
       ...
       double p=rand()%(1000)/(double)(1000);//随机取值0-1
       for(int j=0;j<populationnumber;j++)
       {
           if(p>=roulette[j]&&p<roulette[j+1])//落在相应区间
              a2=j;
       }
       for(int m=0;m<workpiecesnumber;m++)
       {
           a[i][m]=a1[a2][m];//al相当于上一代染色体，a相当于下一代染色体，此处经过轮盘赌选择后在a上进行更新
       }
   }
        ...
        if(n3==2)   //n3=2的概率为0.1；若满足0.1的概率，那么就进行交叉操作；
        {
            ...
            for(int j=0;j<workpiecesnumber;j++)
            {
                ...
                for(int p=n1;p<n2;p++)          //将交叉点位之间的基因片段进行交叉，temp1和temp2记录没有发生重复的基因；
                    {if(a[2*i+1][p]==a[2*i][j])//2*i表示后半段染色体
                      flg1=1;
                    }
